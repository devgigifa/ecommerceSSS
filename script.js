...
function getDados(){
    ...
    // // ARQUIVO (imagem do produto)
    // let arquivo = document.getElementById('arq').files[0];
    // // Você precisa lidar com o upload de arquivo aqui

    return alert("Dados inseridos com sucesso!");
}
...
function montaHTML(){ //********************************************************** */
    ...
    span2.innerHTML = 'Starry Sky Shop'
    ...
    if(usr.includes(loginAut)){
        ...
        for(i in qtd){
            if(qtd[i] > 0){
                ...
                textoCarrinho += qtd[i] + " x " + preco[i].toFixed(2).replace('.', ',') + " - " + produto[i] + " R$ " + totalCompra[i].toFixed(2).replace('.', ',') + "\n"
            }
        }
